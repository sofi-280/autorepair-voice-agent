"""Pipecat pipeline for inbound phone calls (Twilio → Daily SIP → Gemini Live)."""
import argparse
import asyncio
import logging
import sys
...
from pipecat.services.google.live import GoogleLLMService, GoogleLLMContext
...
async def run_bot(room_url: str, room_token: str, call_sid: str, session_id: str) -> None:
    ...
    handlers = ToolHandlers(session_id=session_id, call_sid=call_sid, channel="phone")
...
if __name__ == "__main__":
    ...
    asyncio.run(run_bot(args.room_url, args.room_token, args.call_sid, args.session_id))
